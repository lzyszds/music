...
服务器启动默认端口为 1030, 若不想使用 1030 端口 , 可使用以下命令 : Mac/Linux
...
```javascript
// 例子
const url = `http://localhost:1030/song/url?id=33894312&proxy=http://121.196.226.246:84`
...
docker run -d -p 1030:1030 --name netease_cloud_music_api    binaryify/netease_cloud_music_api


// 或者
docker run -d -p 1030:1030 binaryify/netease_cloud_music_api

// 去掉或者设置相关的环境变量

docker run -d -p 1030:1030 --name netease_cloud_music_api -e http_proxy= -e https_proxy= -e no_proxy= -e HTTP_PROXY= -e HTTPS_PROXY= -e NO_PROXY= binaryify/netease_cloud_music_api

// 或者
docker run -d -p 1030:1030 -e http_proxy= -e https_proxy= -e no_proxy= -e HTTP_PROXY= -e HTTPS_PROXY= -e NO_PROXY= binaryify/netease_cloud_music_api
```
...
$ sudo docker run -d -p 1030:1030 netease-music-api
```
...
`/simi/playlist?id=347230&timestamp=1503019910240` (之所以加入缓存机制是因为项目早期没有缓存机制，很多 issues 都是报 IP 高频，请按自己需求改造缓存中间件(app.js)，源码不复杂)
...
可能会因为没带上 cookie 导致 301, 具体例子可看 `public/test.html`, 访问`http://localhost:1030/test.html`(默认端口的话) 例子使用 jQuery 和 axios
...
调用可参考项目文件例子`/public/qrlogin.html` (访问地址:http://localhost:1030/qrlogin.html)
...
访问地址: http://localhost:1030/cloud.html)
