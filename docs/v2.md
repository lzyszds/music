...
```javascript
// 例子
const url = `http://localhost:1030/music/url?id=33894312&proxy=http://121.196.226.246:84`
...
docker run -d -p 1030:1030 --name netease-cloud-music twesix/netease-music-api

// 去掉或者设置相关的环境变量
docker run -d -p 1030:1030 --name netease-cloud-music -e http_proxy= -e https_proxy= -e no_proxy= -e HTTP_PROXY= -e HTTPS_PROXY= -e NO_PROXY= netease-cloud-music
```
...
`/simi/playlist?id=347230&timestamp=1503019910240`
